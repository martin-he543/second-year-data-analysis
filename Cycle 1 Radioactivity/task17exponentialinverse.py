...
d, u, unc_u = np.loadtxt("Task 17 True.csv", delimiter=",", unpack=True, skiprows=1)
d_restricted, u_restricted, unc_u_restricted = np.loadtxt("Task 17 True.csv", delimiter=",", unpack=True, skiprows=1)

area,unc_area = 1.97e-4, 1.4e-5
def exponential_decay(d,A,mu,k,B):                                                                                 
     return A*area*np.exp(-mu*d)/(4*np.pi*(d+k)**2) + B

params, cov_params = curve_fit(exponential_decay,d,u,p0=[5555555,2.8,-2.45,11.2])
